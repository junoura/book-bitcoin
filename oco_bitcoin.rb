# coding: utf-8
# check_sell_bitcoin.rb
# 価格が指定した価格になるのを待ってbitcoinを成行で売却する
# 引数は売却に使用するBitcoinの金額と価格(利益確定と損切り)
...
if ARGV.length != 3
  puts "USAGE: ruby #{$0} 売却に使用するBitcoinの金額 Bitcoinの利益確定単価 Bitcoinの損切り単価"
  exit
end
...
rate_of_btc_profit = ARGV[1].to_f
rate_of_btc_loss = ARGV[2].to_f
...
if rate_of_btc_profit > rate_of_btc * 1.1
  puts "指定した価格が高すぎます"
  log.error("指定した価格が高すぎます")
  exit
end
if rate_of_btc_profit < rate_of_btc * 0.9
  puts "指定した価格が低すぎます"
  log.error("指定した価格が低すぎます")
  exit
end
if rate_of_btc_loss > rate_of_btc * 1.1
  puts "指定した価格が高すぎます"
  log.error("指定した価格が高すぎます")
  exit
end
if rate_of_btc_loss < rate_of_btc * 0.9
  puts "指定した価格が低すぎます"
  log.error("指定した価格が低すぎます")
  exit
end
...
loop do  
  ...
  # 取得された「価格」が指定された「利益確定価格」より大きい場合、売却します。
  if rate_of_btc > rate_of_btc_profit
    ...
    if response_success
      ...
      puts "#{amount_of_btc}BTCを成行で売却(利益確定)注文しました。"
      log.info("#{amount_of_btc}BTCを成行で売却(利益確定)注文しました。")
    else
    ...
    puts "指定した利益確定価格に達していません(現在:#{rate_of_btc} 目標:#{rate_of_btc_profit})"
    log.info("指定した利益確定価格に達していません(現在:#{rate_of_btc} 目標:#{rate_of_btc_profit})")
  end
  # 取得された「価格」が指定された「損切り価格」より小さい場合、売却します。
  if rate_of_btc < rate_of_btc_loss
    ...
    if response_success
      ...
      response_amount = response_hash['amount']
      response_created_at = response_hash['created_at']
      puts "#{amount_of_btc}BTCを成行で売却(損切り)注文しました。"
      log.info("#{amount_of_btc}BTCを成行で売却(損切り)注文しました。")
    else
      response_error = response_hash['error']
      puts "error = #{response_error}"
      log.error("error:#{response_error}")
