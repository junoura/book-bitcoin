# coding: utf-8
# ifd_bitcoin.rb
# 価格が指定した価格になるのを待ってbitcoinを購入し、価格が指定した価格になるのを待ってbitcoinを売却する
# 引数は売却に使用するBitcoinの金額と価格(購入と売却)
...
if ARGV.length != 3
  puts "USAGE: ruby #{$0} 日本円の金額 Bitcoinの購入単価 Bitcoinの売却単価"
  exit
end
...
if rate_of_btc_sell > estimated_rate * 1.1
  puts "指定した価格が高すぎます"
  log.error("指定した価格が高すぎます")
  exit
end
if rate_of_btc_sell < estimated_rate * 0.9
  puts "指定した価格が低すぎます"
  log.error("指定した価格が低すぎます")
  exit
end
...
loop do  
  ...
  rate_of_btc = response_hash['rate'].to_f
  # 指定された「価格」が取得された「価格」より1割以上差がある場合、エラーとします。
  if rate_of_btc > rate_of_btc_sell
    puts "指定した売却価格に達しました(現在:#{rate_of_btc} 目標:#{rate_of_btc_sell})"
    log.info("指定した売却価格に達しました(現在:#{rate_of_btc} 目標:#{rate_of_btc_sell})")
    # 実際の売買を行います。
    ...
    puts "指定した売却価格に達していません(#{Time.now} 現在:#{rate_of_btc} 目標:#{rate_of_btc_sell})"
  end
